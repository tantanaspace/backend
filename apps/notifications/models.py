...
class NotificationTemplate(AbstractTimeStampedModel):
    ...
    type = models.CharField(_("Type"), max_length=50, choices=TYPE_CHOICES, unique=True)
    ...
    def clean(self):
        ...
        is_complete_title = all(
            getattr(self, field) for field in get_translation_fields("title")
        )
        is_complete_body = all(
            getattr(self, field) for field in get_translation_fields("body_text")
        )
