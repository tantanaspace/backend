...
from apps.notifications.models import CustomFCMDevice, Notification, UserNotification
...
class FirebaseMessaging:
    ...
    @staticmethod
    def __create_apns_config(
        title: str, body: str, image: str | None = None, badge: int = 0
    ):
        """
        ...
        aps_alert = messaging.ApsAlert(
            title=title, body=body, launch_image=None, custom_data=None
        )
        ...
        apns_payload = messaging.APNSPayload(aps=aps, custom_data=None)

        apns_fcm_options = messaging.APNSFCMOptions(analytics_label=None, image=image)

        apns_config = messaging.APNSConfig(
            headers=None, payload=apns_payload, fcm_options=apns_fcm_options
        )

        return apns_config

    @staticmethod
    def __create_android_config(
        title: str, body: str, image: str | None = None, badge: int = 0
    ):
    ...
    def _send_message(self, message: messaging.Message) -> Dict[str, Union[str, bool]]:
        ...
        try:
            ...
            return {"success": True, "message_id": message_id, "token": message.token}
        except messaging.exceptions.FirebaseError as e:
            error_msg = f"Firebase error while sending message: {str(e)}"
            logger.error(error_msg)
            if self.raise_exception:
                raise Exception(error_msg)
            return {"success": False, "error": error_msg, "token": message.token}
    ...
    def _send_each_for_multicast(
        self, multicast_message: messaging.MulticastMessage
    ) -> Dict[str, Union[List[Dict], str]]:
        """
        ...
        try:
            response = messaging.send_each_for_multicast(
                multicast_message, app=self.firebase
            )
            results = []
            ...
            for idx, result in enumerate(response.responses):
                ...
                if result.success:
                    ...
                    results.append(
                        {"success": False, "error": result.exception, "token": token}
                    )
    ...
    def create_multicast_messages(
        ...
        if not (title and body and tokens):
            raise Exception(
                "Title, body, and tokens are required to create multicast messages."
            )
...
class NotificationSender:
    ...
    def _validate_notification_status(self) -> None:
        if self.notification.status == Notification.Status.IN_PROCESS:
            raise ValueError(
                "This notification sending task is in progress. Already running."
            )
    ...
    def _send_multicast_message(self, lang: str, tokens: List[str]) -> None:
        ...
        body = getattr(
            self.notification, f"body_text_{lang}", self.notification.body_text
        )
    ...
    def _process_multicast_results(
        self, results: List[Dict], tokens: List[str]
    ) -> None:
        for idx, token_result in enumerate(results):
            ...
            if token_result.get("success"):
                ...
                logger.error(
                    f"Failed to send to token {token}: {token_result.get('error')}"
                )
        ...
        if not self.only_push:
            ...
            with transaction.atomic():
                ...
                UserNotification.objects.filter(
                    notification=self.notification, user_id__in=successful_user_ids
                ).update(is_sent=True, sent_at=now)

    def _get_result_stats(
        self, user_notifications: List[UserNotification], total_tokens: int
    ) -> Dict:
        return {
    ...
    def send(self) -> Dict:
        try:
            ...
            result = self._get_result_stats(
                user_notifications,
                sum(len(tokens) for tokens in tokens_by_language.values()),
            )
