...
from apps.common.services.eskiz import send_sms, eskiz_interface
from apps.api.shared.v1.users.Auth.RequestOTP.serializers import RequestOTPSerializer, UserAction
from utils.generators import generate_code, generate_cache_key, CacheType, CodeType
...
class RequestOTPAPIView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (action == UserAction.REGISTRATION and user_exists) or action == UserAction.LOGIN:
            response_data = {
        ...
        existing_cache_key = generate_cache_key(self.cache_type_map[action], str(phone_number))
        existing_cache_data = cache.keys(f'{existing_cache_key}*')

        if existing_cache_data:
            raise ValidationError({"phone_number": "You can request only once 2 minute."}, code='cooldown')
        ...
        if not is_sent:
            ...
            raise ValidationError({"phone_number": "Failed to send SMS. Please try again."}, code='sms_failed')
