...
from apps.api.shared.v1.common import ConfigDetailAPIView, EskizCallbackAPIView
...
urlpatterns = [
    ...
    path(
        ...
        ConfirmationOTPAPIView.as_view(),
    ...
    path(
        "create-device/",
        FCMViewSet.as_view({"post": "create"}),
        name="create_fcm_device",
    ),
    path(
        "delete-device/<str:registration_id>/",
        FCMViewSet.as_view({"delete": "destroy"}),
        name="delete_fcm_device",
    ),
    path(
        "update-device/<str:registration_id>/",
        FCMViewSet.as_view({"patch": "partial_update"}),
        name="update_fcm_device",
    ),
    # common.options
    path(
        "options/regions/", RegionOptionListAPIView.as_view(), name="region-option-list"
    ),
    path(
        "options/countries/",
        CountryOptionListAPIView.as_view(),
        name="country-option-list",
    ),
    path(
        "options/facilities/",
        FacilityListAPIView.as_view(),
        name="facility-option-list",
    ),
    path("options/tags/", TagListAPIView.as_view(), name="tag-option-list"),
    path(
        "options/venue-categories/",
        VenueCategoryListAPIView.as_view(),
        name="venue-category-option-list",
    ),
    # common
