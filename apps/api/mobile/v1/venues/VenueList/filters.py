...
class VenueListFilter(filters.FilterSet):
    ...
    min_rating = filters.NumberFilter(
        field_name="rating", lookup_expr="gte", help_text="Minimum rating (0.0 - 5.0)"
    )

    max_rating = filters.NumberFilter(
        field_name="rating", lookup_expr="lte", help_text="Maximum rating (0.0 - 5.0)"
    )

    # Company filter
    company = filters.NumberFilter(
        field_name="company", lookup_expr="exact", help_text="Filter by company ID"
    )
