...
class GooglePlacesClient:
    ...
    def __init__(self):
        ...
        self.headers = {"X-Goog-Api-Key": self.api_key, "X-Goog-FieldMask": "places.id"}
            ...
            for place_type in place_types:
                ...
                payload = {
                    "locationRestriction": {
                        "circle": {
                            "center": {
                                "latitude": tashkent_grid_coordinates[coordinate][0],
                                "longitude": tashkent_grid_coordinates[coordinate][1],
                            },
                            "radius": 7000.0,
                        }
                    },
    ...
    def _text_search(self) -> List[Dict]:
        ...
        search_queries = [
            ...
            "tea houses in Tashkent",
            # National cuisines
            "uzbek restaurants in Tashkent",
            "turkish restaurants in Tashkent",
            ...
            "american restaurants in Tashkent",
            "mediterranean restaurants in Tashkent",
            "middle eastern restaurants in Tashkent",
            "barbecue restaurants in Tashkent",
            "steak houses in Tashkent",
            "sushi restaurants in Tashkent",
            "vegan restaurants in Tashkent",
            "vegetarian restaurants in Tashkent",
            # Districts
            "restaurants in Chilonzor Tashkent",
            "cafes in Chilonzor Tashkent",
            "restaurants in Yunusobod Tashkent",
            "cafes in Yunusobod Tashkent",
            "restaurants in Mirzo Ulugbek Tashkent",
            "cafes in Mirzo Ulugbek Tashkent",
            "restaurants in Shaykhantahur Tashkent",
            "cafes in Shaykhantahur Tashkent",
            "restaurants in Yakkasaroy Tashkent",
            "cafes in Yakkasaroy Tashkent",
            "restaurants in Sergeli Tashkent",
            "cafes in Sergeli Tashkent",
            "restaurants in Bektemir Tashkent",
            "cafes in Bektemir Tashkent",
            "restaurants in Olmazor Tashkent",
            "cafes in Olmazor Tashkent",
            # Popular places
            "restaurants near Tashkent airport",
            "cafes near Tashkent airport",
            "restaurants near Tashkent railway station",
            "cafes near Tashkent railway station",
            "restaurants near Magic City Tashkent",
            "cafes near Magic City Tashkent",
            "restaurants near Samarkand Darvoza mall Tashkent",
            "cafes near Samarkand Darvoza mall Tashkent",
            "restaurants near Next mall Tashkent",
            "cafes near Next mall Tashkent",
            "restaurants near Compass mall Tashkent",
            "cafes near Compass mall Tashkent",
            "restaurants near Tashkent City Park",
            "cafes near Tashkent City Park",
            "restaurants near Chorsu Bazaar Tashkent",
            "cafes near Chorsu Bazaar Tashkent",
            "restaurants near Amir Temur Square Tashkent",
            "cafes near Amir Temur Square Tashkent",
        ]
        ...
        for search_query in search_queries:
            ...
            try:
                ...
                if "places" in data:
                    results.extend(data["places"])
                    logger.info(
                        f"Found {len(data['places'])} places for query '{search_query}'"
                    )
    ...
    def get_place_details(self, place_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning(
                    f"API response for {place_id}: {response.status_code} - {response.text}"
                )
            ...
            if not isinstance(data, dict):
                logger.warning(
                    f"Unexpected response format for {place_id}: {type(data)}"
                )
                return None
...
class VenueDataProcessor:
    ...
    def process_venue(self, place_data: Dict) -> Optional[Venue]:
        ...
        try:
            ...
            self._process_working_hours(
                venue, place_data.get("regularOpeningHours", {})
            )
    ...
    def _process_categories(self, venue: Venue, place_data: Dict):
        ...
        try:
            ...
            for type_name in types:
                category_title = (
                    slugify(type_name).replace("-", " ").replace("_", " ").title()
                )
        ...
        for period in periods:
            ...
            if open_time and close_time:
                try:
                    ...
                    logger.warning(
                        f"Failed to create working hours for {venue.name} on {weekday}: {e}"
                    )
        ...
        for i, photo in enumerate(photos[:5]):  # Max 5 photos
            try:
                ...
                if not photo_content:
                    logger.warning(
                        f"Failed to download photo {photo_name} for venue {venue.name}"
                    )
                    continue
                ...
                image_obj, _ = VenueImage.objects.get_or_create(
                    ...
                    defaults={"order": i + 1, "is_main": (i == 0)},
                )
    ...
    def _extract_business_features(self, place_data: Dict) -> Dict:
        ...
        features["serves_breakfast"] = self._safe_get_bool(
            place_data, "servesBreakfast"
        )
        ...
        features["serves_cocktails"] = self._safe_get_bool(
            place_data, "servesCocktails"
        )
        ...
        features["good_for_children"] = self._safe_get_bool(
            place_data, "goodForChildren"
        )
        features["good_for_groups"] = self._safe_get_bool(place_data, "goodForGroups")
        features["good_for_watching_sports"] = self._safe_get_bool(
            place_data, "goodForWatchingSports"
        )
    ...
    try:
        ...
        for i, place_item in enumerate(search_results, 1):
            try:
                ...
                place_details = google_places_client.get_place_details(
                    place_item.get("id")
                )
                ...
                logger.info(
                    f"Processed {i}/{len(search_results)}: {venue.name if venue else 'Failed'}"
                )
        ...
        logger.info(
            f"Venue synchronization completed successfully. Processed {processed_count} venues at {now()}"
        )
