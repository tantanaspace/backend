...
class User(AbstractUser, AbstractSoftDeleteModel):
    ...
    language = models.CharField(
        _("Language"), max_length=6, choices=Language.choices, default=Language.UZBEK
    )
    avatar = models.ImageField(
        _("Avatar"), upload_to="users/avatar", null=True, blank=True
    )
    ...
    venue = models.ForeignKey(
        "venues.Venue", on_delete=models.CASCADE, null=True, blank=True
    )
    ...
    def delete(self, using=None, keep_parents=False):
        ...
        self.save(
            update_fields=["is_deleted", "deleted_at", "phone_number", "is_active"]
        )
