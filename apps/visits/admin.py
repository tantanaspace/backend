...
class VisitAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_("Host Information"), {"fields": ("host",), "classes": ("collapse",)}),
        (
            _("Timeline"),
            {
                "fields": (
        ...
        (_("Guests"), {"fields": ("guests_count",), "classes": ("collapse",)}),
    )
    ...
    def get_queryset(self, request):
        return (
            ...
            .prefetch_related("guests", "guests__user", "order", "order__items")
        )
...
class VisitGuestAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Guest Information"), {"fields": ("user", "visit")}),
        (_("Join Status"), {"fields": ("is_joined", "joined_at")}),
    )

    def user_info(self, obj):
        url = reverse("admin:users_user_change", args=[obj.user.id])
        return format_html(
            '<a href="{}">{} ({})</a>', url, obj.user.full_name, obj.user.phone_number
        )
    ...
    def visit_status(self, obj):
        ...
        return format_html(
            '<span style="color: {};">{}</span>', color, obj.visit.get_status_display()
        )
    ...
    def get_queryset(self, request):
        return (
            super()
            .get_queryset(request)
            .select_related("user", "visit", "visit__venue")
        )
