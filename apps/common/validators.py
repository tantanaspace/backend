    ...
    def __call__(self, value):
        ...
        try:
            ...
            raise ValidationError(
                self.message or str(e),
                code=self.code or 'invalid'
            )
...
def file_validator(
    ...
    def _validator(file):
        if file.size > max_size_mb * 1024 * 1024:
            raise ValidationError(_(f"File size must be under {max_size_mb}MB"), code='file_size')

        if allowed_mime_types:
            mime_type = magic.from_buffer(file.read(2048), mime=True)
            file.seek(0)
            if mime_type not in allowed_mime_types:
                raise ValidationError(_(f"Unsupported file type: {mime_type}"), code='mime_type')

        if allowed_extensions:
            ext = file.name.split('.')[-1].lower()
            if ext not in allowed_extensions:
                raise ValidationError(f"Unsupported file extension: .{ext}", code='extension')
