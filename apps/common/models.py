...
class GlobalSettings(SingletonModel):
    ...
    backoffice_url = models.URLField(
        _("Backoffice URL"), help_text=_("URL of the backoffice"), blank=True, null=True
    )
...
class CompanyProfile(SingletonModel):
    ...
    support_email = models.EmailField(
        verbose_name=_("Support Email"), help_text=_("General support email address")
    )
    support_telegram_link = models.URLField(_("Support Telegram Link"))
    support_phone_number = PhoneNumberField(_("Phone Number"))

    about = models.TextField(
        verbose_name=_("About"), help_text=_("Information about the company team")
    )
...
class Facility(AbstractTimeStampedModel):
    ...
    icon = VersatileImageField(
        _("Icon"), upload_to="facilities/icons/", blank=True, null=True
    )
...
class StoryItem(AbstractTimeStampedModel):
    ...
    def save(self, *args, **kwargs):
        if not self.order:
            ...
            last_order = (
                StoryItem.objects.filter(story_group=self.story_group).aggregate(
                    models.Max("order")
                )["order__max"]
                or 0
            )
