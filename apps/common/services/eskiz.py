...
class EskizInterface:
    ...
    def access_token(self):
        ...
        if eskiz_access_token_data:
            try:
                ...
                expire_time = timezone.datetime.fromisoformat(
                    eskiz_access_token_data.get("expire_time", "")
                )
        ...
        if not eskiz_access_token_data or expire_time < timezone.now():
            ...
            cache.set(
                self.cache_key,
                json.dumps({"access_token": access_token, "expire_time": expire_time}),
                timeout=30 * 24 * 60 * 60,
            )
    ...
    def send_sms(self, phone_number, message):
        ...
        response = requests.post(
            f"{self.base_url}/api/message/sms/send", headers=headers, json=data
        )
        return response
...
def send_sms(phone_number: PhoneNumber, message: str):
    ...
    if "test" not in sys.argv:
        ...
        try:
            ...
            is_sent = False
            response_log = {"error": str(e), "exception_type": type(e).__name__}
