...
class PaymentTransactionLoggingMiddleware:
    ...
    def process_request(self, request):
        if request.path in self.provider_urls.keys():
            ...
            try:
                ...
                method = (
                    "prepare"
                    if str(request.path).startswith("/api/v1/click-prepare/")
                    else "complete"
                )
    ...
    def process_response(self, request, response):
        if request.path in self.provider_urls.keys() and hasattr(
            self._local, "payment_log_id"
        ):
            try:
                payment_log = PaymentRequestLog.objects.get(
                    id=self._local.payment_log_id
                )
