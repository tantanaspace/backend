...
from apps.common.models import AbstractSoftDeleteModel, AbstractTimeStampedModel
...
class PaymentTransaction(AbstractTimeStampedModel):
    ...
    user = models.ForeignKey(
        "users.User", on_delete=models.PROTECT, related_name="payment_transactions"
    )
    visit = models.ForeignKey(
        "visits.Visit", on_delete=models.PROTECT, related_name="payment_transactions"
    )
    ...
    def payment_url(self):
        ...
        if self.provider == PaymentTransaction.ProviderType.PAYME:
            merchant_id = settings.PAYMENT_CREDENTIALS.get("payme", {}).get(
                "merchant_id"
            )
            ...
            merchant_id = settings.PAYMENT_CREDENTIALS.get("paylov", {}).get(
                "merchant_id"
            )
            ...
            merchant_id = settings.PAYMENT_CREDENTIALS.get("click", {}).get(
                "merchant_id"
            )
            service_id = settings.PAYMENT_CREDENTIALS.get("click", {}).get(
                "merchant_service_id"
            )
            params = (
...
class PaymentRequestLog(AbstractTimeStampedModel):
    ...
    method = models.CharField(
        max_length=255, verbose_name=_("Method"), null=True, blank=True
    )
    request_data = models.JSONField(
        verbose_name=_("Request Data"), null=True, blank=True
    )
    response_data = models.JSONField(
        verbose_name=_("Response Data"), null=True, blank=True
    )
