...
class OrderItemInline(admin.TabularInline):
    model = OrderItem
    extra = 0
    fields = ('product_name', 'quantity', 'unit_price', 'total_price', 'status', 'ordered_at', 'served_at')
...
class OrderAdmin(admin.ModelAdmin):
    list_display = (
        'id', 'visit_info', 'waiter_full_name', 'total_amount', 
        'service_fee', 'percentage_of_service', 'items_count', 'created_at'
    )
    list_display_links = ('id', 'visit_info')
    list_filter = (
        'created_at', 'updated_at', 'visit__status', 'visit__venue'
    )
    search_fields = (
        'id', 'visit__id', 'visit__venue__name', 'visit__user_full_name',
        'visit__user__full_name', 'waiter_full_name'
    )
    ordering = ('-created_at',)
    ...
    fieldsets = (
        (_('Order Information'), {
            'fields': ('visit', 'waiter_full_name')
        }),
        (_('Financial Details'), {
            'fields': (
                'percentage_of_service', 'service_fee', 'total_amount', 
                'total_calculated'
            )
        }),
        (_('Items Summary'), {
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).prefetch_related(
            'items', 'visit__venue', 'visit__user'
        )
...
class OrderItemAdmin(admin.ModelAdmin):
    list_display = (
        'id', 'product_name', 'order_info', 'quantity', 'unit_price', 
        'total_price', 'status', 'ordered_at', 'served_at'
    )
    list_display_links = ('id', 'product_name')
    list_filter = (
        'status', 'created_at', 'ordered_at', 'served_at', 
        'order__visit__venue', 'order__visit__status'
    )
    search_fields = (
        'product_name', 'order__id', 'order__visit__id', 
        'order__visit__venue__name', 'order__waiter_full_name'
    )
    ordering = ('-created_at',)
    readonly_fields = ('created_at', 'updated_at', 'total_price', 'ordered_at', 'served_at', 'cancelled_at')
    ...
    def order_info(self, obj):
        ...
        return format_html(
            '<a href="{}">Order #{} - {}</a>',
            url, obj.order.id, obj.order.visit.venue.name
        )
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related(
            'order', 'order__visit', 'order__visit__venue'
        )
